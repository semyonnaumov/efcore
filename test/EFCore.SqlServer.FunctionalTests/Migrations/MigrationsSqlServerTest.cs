...
""");
    ...
    [ConditionalFact]
    public virtual async Task Adding_sparse_column_to_temporal_table()
    {
        await Test(
            builder => builder.Entity(
                "Customer", e =>
            ...
            builder => builder.Entity(
                "Customer", e =>
                {
                    e.Property<int?>("MySparseColumn").IsSparse();
                }),
            model =>
            {
                var table = Assert.Single(model.Tables);
                ...
                Assert.Collection(
                    ...
                    c => Assert.Equal("Id", c.Name),
                    c => Assert.Equal("Name", c.Name),
                    c => Assert.Equal("MySparseColumn", c.Name));
                Assert.Same(
                    table.Columns.Single(c => c.Name == "Id"),
                    Assert.Single(table.PrimaryKey!.Columns));
            });
        ...
        AssertSql(
"""
ALTER TABLE [Customer] ADD [MySparseColumn] int SPARSE NULL;
""");
    }
