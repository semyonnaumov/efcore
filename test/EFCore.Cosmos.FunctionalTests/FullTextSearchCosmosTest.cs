...
""");
    ...
    public virtual async Task Use_FullTextContains_in_predicate_on_nested_owned_type()
    {
        ...
        var result = await context.Set<FullTextSearchAnimals>()
            .Where(x => EF.Functions.FullTextContains(x.Owned.Nested.AnotherDescription, "beaver"))
            .ToListAsync();
...
SELECT VALUE c
FROM root c
WHERE FullTextContains(c["Owned"]["Nested"]["AnotherDescription"], "beaver")
""");
    }

    [ConditionalFact]
    public virtual async Task OrderByRank_with_FullTextContains_on_nested_owned_type()
    {
        await using var context = CreateContext();

        var result = await context.Set<FullTextSearchAnimals>()
            .OrderBy(x => EF.Functions.FullTextScore(x.Owned.Nested.AnotherDescription, new string[] { "beaver", "dolphin" }))
            .ToListAsync();
...
SELECT VALUE c
FROM root c
ORDER BY RANK FullTextScore(c["Owned"]["Nested"]["AnotherDescription"], ["beaver","dolphin"])
""");
    }
    ...
    {
        public FullTextSearchNested Nested { get; set; } = null!;
    }
    ...
    public class FullTextSearchFixture : SharedStoreFixtureBase<PoolableDbContext>
    {
        ...
        protected override Task SeedAsync(PoolableDbContext context)
        {
            var landAnimals = new FullTextSearchAnimals
            {
                ...
                {
                    Nested = new FullTextSearchNested
                    {
