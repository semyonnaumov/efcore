...
""");
    ...
    [ConditionalFact]
    public virtual async Task Add_computed_column_to_temporal_table()
    {
        await Test(
            builder => builder.Entity(
                "Customer", e =>
            ...
            builder => builder.Entity(
                "Customer", e =>
                {
                    e.Property<int>("Level").HasComputedColumnSql("Id.GetLevel() PERSISTED");
                }),
            model =>
            {
                var table = Assert.Single(model.Tables);
