...
public class StructuralTypeShaperExpression : Expression, IPrintableExpression
{
    ...
    protected virtual LambdaExpression GenerateMaterializationCondition(ITypeBase type, bool nullable, ILiftableConstantFactory liftableConstantFactory)
    {
        ...
        else
        {
            ...
            var entityTypeName = (concreteEntityTypes.Length == 1 ? concreteEntityTypes[0] : entityType).Name;
            body = liftableConstantFactory.CreateLiftableConstant(c => c.Dependencies.Model.FindEntityType(entityTypeName)!, entityTypeName + "EntityType", typeof(IEntityType));
        }
